-- DML : select, insert, update, dlete
--           테이블의 행을 대상으로 하는 명령어
...
-- 주위 : update 와 delete 는 where 없이 사용하면 위험.!!!

-- insert, update, delete 는 변경사항에 대해 바로 테이블에 반영(commit) 하지 않고
...
--              autocommit 설정이 false 이면 다른 세션(클라이언트-sqlplus, beaver, ...)
--                              ㄴ commit 하기전 까지는 변경된 데이터를 확인 할 수 없습니다.(반영❌)
