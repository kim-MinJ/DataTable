...
FROM tbl_student, tbl_score;
 -- tbl_student 5행, btl_score : 9행 데이터 있음. 5*9=45 행이 만들어짐.(모든 경우의 수)

-- 중복 컬럼 stuno : 위의 크로스 조인 결과 중에서 stuno 값이 같은 것만 조회
-- 예상되는 행의 갯수 ?
SELECT *
FROM tbl_student stu, tbl_score sco  -- stu, sco 각 테이블의 별칭
WHERE stu.STUNO = sco.STUNO;  -- tbl_student.STUNO = tbl_score.STUNO
-- tbl.student.STUNO = tbl_score.STUNOALTER
...
SELECT stu.Name, stu.STUNO, sco.JUMSU, sco.SUBJECT
FROM TBL_STUDENT stu, TBL_SCORE sco
...
-- 실행 될 때에 서브쿼리 값  2025003, 2025004, 2024004, 2025005
...
-- step 3 :  step 2 결과와 TBL_STUDENT 와 join
SELECT stu.NAME, "AVG"
