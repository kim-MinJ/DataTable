...
-- 참고 : nvl 함수, decode 함수, case ~ when ~end 예시 찾아보세요.


-- 위의 쿼리를 자주 사용한다면 미리 컴파일하여 view 객체로 만듭니다.
-- 그러면 좀 더 쿼리가 빠르고 단순해 집니다.
-- view 는 create or replace 명령을 사용하여 수정이 필요할 때 drop 없이 다시 정의
create or replace view v_final_sale
as SELECT tp.pcode, tp.pname, tp.price*sale.salesum AS total
FROM TBL_PRODUCT tp 
JOIN 
  (
    SELECT PCODE, SUM(QUANTITY)  AS salesum 
