-- 그룹화 연습을 위해 tbl_car_co2 에 행 데이터 변경(최종 31개) 아래에 있음
/*
그룹화 : 지정된 컬럼값이 같은 행들을 하나의 그룹으로 묶고
            집계 함수를 그룹 단위로 적용.
            지정된 컬럼은 그룹화 컬럼 : 대체로 같은 값이(중복)이 많은 컬럼들이 대상이 됩니다.
            그룹화 컬럼에 대해 집계 함수를 적용.

            SELECT 그룹화컬럼, 함수(Number 컬럼)
            FROM 테이블명
...
-- 요구사항 : 자동차 브랜드별로(GROUP) co2평균을 구하고 평균값이 100미만 조회
...
-- 평균값이 1000을 초과한 것 조회
SELECT VOLUME, MIN(CAR) AS CAR, AVG(WEIGHT)
FROM TBL_CAR_CO2
WHERE CO2 < 100
GROUP BY VOLUME
...
-- 2개 이상 컬럼으로 그룹화
SELECT CAR, MODEL, COUNT(*) AS "개수", ROUND(AVG(C02)) AS "CO2 평균"
FROM TBL_CAR_CO2
GROUP BY CAR, MODEL;  
-- car 컬럼에 동일한 값이 많고, model 컬럼도 동일한 값이 많을 때
-- (CAR, MODEL) 페어. 1차 그룹화 car 컬럼, 1차 그룹 안에서 2차 그룹 model 컬럼
