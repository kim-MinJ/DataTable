  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < 50; i++) {
      ...
      executor.submit(() -> {
        ...
        // DBCP 는 5~10개 size
        try {
          long connEnd = System.currentTimeMillis(); // 커넥션 생성 종료 시간 (무시 : 삭제해서 없음)
          ProductVo vo = dao.selectByPk("APLE5kg");
          long totalEnd = System.currentTimeMillis(); // dao sql 실행 종료 시간
 ...
 * 초과된 작업은 큐에 저장되어 대기하다가 스레드가 사용가능 해지면 실행됩니다.
 * 자료구조
 * : Stack 은 LIFO (마지막입력값을 가장 먼저 처리) , 기본 연산 처리
