...
public class ResultSetTest {
    ...
    public static void selectByLevel() { // 같은 레벨(step) 값이 여러 개 있으므로 조회 되는 행은 0 ~ 여러개
        ...
        try (
            ...
            pstat.setString(1, step); // 1은 매개변수의 위치
            ResultSet rs = pstat.executeQuery();
            // 🔥 SELECT 실행 메소드 executeQuery 실행결과(행 집합)는 Resultset 타입
            boolean result = rs.next(); // 처음 rs 커서는 첫번째 앞에 있으므로 다음 이동 필요.
            int count = 0;
            while (result) {
                count++;
                // 행집합 안에서 커서(행 위치)를 다음으로 이동. rs.next() 리턴이 false 이면 결과 행이 없음.
    ...
    public static void selectByPk() { // 조회 결과 0 또는 1개 행
        System.out.println("-----------tbl_javadict 테이블 조회하기 -----------");
        String english = System.console().readLine("조회 영단어 입력 >>> ");
        ...
        try (
            ...
            System.out.println("조회 결과 있음 ?" + result); // 조회 결과 있음 ?true
            if (result) {
                System.out.println("#### 조회 결과 컬럼값 출력 ###");
